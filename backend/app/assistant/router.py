...
async def _build_context(db: AsyncSession, user_id: str, engagement_id: Optional[str], question: str) -> tuple[str, list[str]]:
    ...
    if engagement_id:
        ...
        if eng:
            ...
            if findings:
                findings_text = "\n".join(
                    f"  [{f.severity.value.upper() if hasattr(f.severity, 'value') else f.severity.upper()}] "
            ...
            if any(kw in question_lower for kw in ['attack', 'path', 'chain', 'exploit', 'lateral', 'escalat']):
                ...
                if paths:
                    path_text = "\n\n".join(
                        f"Path: {p.name} (Risk: {p.risk_level or 'N/A'})\n"
        ...
        eng_result = await db.execute(select(Engagement).order_by(Engagement.created_at.desc()))
        ...
        if engagements:
            ...
            for e in engagements:
                if e.name.lower() in question_lower or e.client_name.lower() in question_lower:
                    ...
                    sub_context, sub_labels = await _build_context(db, user_id, e.id, question)
                    context_parts.append(sub_context)
                    context_labels.extend(sub_labels)
                    break

    return "\n\n".join(context_parts), context_labels
...
async def chat(
    ...
    # Build context
    context, labels = await _build_context(db, current_user.id, body.engagement_id, body.message)

    # Build the prompt
    user_message = body.message
    if context:
        user_message = f"Context from the user's data:\n\n{context}\n\nUser question: {body.message}"
    ...
    return ChatResponse(response=response, context_used=labels)
